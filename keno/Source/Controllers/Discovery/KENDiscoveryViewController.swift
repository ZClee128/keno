...
class KENDiscoveryViewController: UIViewController, UICollectionViewDataSource, UICollectionViewDelegate {
    
    private var collectionView: UICollectionView!

    
    // Data Source Arrays
    private var spotlightPosts: [KENPost] = []
    private var vibes: [String] = []
    private var editorialPosts: [KENPost] = []
    ...
    private func setupCollectionView() {
        ...
        // Register Cells
        collectionView.register(KENDiscoveryPostCell.self, forCellWithReuseIdentifier: "PostCell")
        collectionView.register(KENDiscoveryCapsuleCell.self, forCellWithReuseIdentifier: "CapsuleCell")
        collectionView.register(KENSectionHeaderView.self, forSupplementaryViewOfKind: UICollectionView.elementKindSectionHeader, withReuseIdentifier: "Header")
    ...
    private func createLayout() -> UICollectionViewLayout {
        return UICollectionViewCompositionalLayout { (sectionIndex, env) -> NSCollectionLayoutSection? in
            let sectionKind = KENDiscoverySection.allCases[sectionIndex]
    ...
    func numberOfSections(in collectionView: UICollectionView) -> Int {
        return KENDiscoverySection.allCases.count
    }
    
    func collectionView(_ collectionView: UICollectionView, numberOfItemsInSection section: Int) -> Int {
        let sectionKind = KENDiscoverySection.allCases[section]
        switch sectionKind {
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        let sectionKind = KENDiscoverySection.allCases[indexPath.section]
        switch sectionKind {
        case .spotlight:
            let cell = collectionView.dequeueReusableCell(withReuseIdentifier: "PostCell", for: indexPath) as! KENDiscoveryPostCell
            cell.configure(with: spotlightPosts[indexPath.item])
            return cell
        case .vibes:
             let cell = collectionView.dequeueReusableCell(withReuseIdentifier: "CapsuleCell", for: indexPath) as! KENDiscoveryCapsuleCell
    ...
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        ...
        case .vibes:
            ...
            let tagFeedVC = KENTagFeedViewController()
    ...
    func collectionView(_ collectionView: UICollectionView, viewForSupplementaryElementOfKind kind: String, at indexPath: IndexPath) -> UICollectionReusableView {
        let header = collectionView.dequeueReusableSupplementaryView(ofKind: kind, withReuseIdentifier: "Header", for: indexPath) as! KENSectionHeaderView
        let section = KENDiscoverySection.allCases[indexPath.section]
...
class KENDiscoveryPostCell: UICollectionViewCell {
    ...
    override init(frame: CGRect) {
        ...
        label.font = KENConstants.Fonts.caption(size: 12)
...
class KENDiscoveryCapsuleCell: UICollectionViewCell {
    private let label = UILabel()
    ...
    override init(frame: CGRect) {
        ...
        contentView.layer.borderColor = KENConstants.Colors.reptileGreen.withAlphaComponent(0.3).cgColor
        
        contentView.addSubview(label)
        label.pin(to: contentView, insets: UIEdgeInsets(top: 8, left: 16, bottom: 8, right: 16))
        
        label.font = KENConstants.Fonts.body(size: 14)
    ...
    override var isSelected: Bool {
        didSet {
            UIView.animate(withDuration: 0.3) {
                self.contentView.backgroundColor = self.isSelected ? KENConstants.Colors.reptileGreen : KENConstants.Colors.cardBackground
                self.label.textColor = self.isSelected ? .black : KENConstants.Colors.textPrimary
            }
        }
    }
