...
class WLDFeedController {
    static let shared = WLDFeedController()
    ...
    private init() {
        // Load from disk
        let savedPosts: [WLDArticle] = WLDStorageWorker.shared.load(from: kPostsFile) ?? []
        let seededPosts = WLDInitialData.getPosts()
    ...
    func addPost(image: UIImage, caption: String) {
        let user = WLDAuthService.shared.currentUser
        
        let imageFileName = "post_\(UUID().uuidString).jpg"
        let imagePath = WLDStorageWorker.shared.saveImage(image, fileName: imageFileName) ?? ""
        
        let newPost = WLDArticle(
            id: UUID().uuidString,
        ...
        // SAVE TO DISK
        WLDStorageWorker.shared.save(posts, to: kPostsFile)
        
        // Notify
        NotificationCenter.default.post(name: NSNotification.Name("WLDNewPostAdded"), object: nil)
    }
    ...
    func deleteUserPosts(userId: String) {
        ...
        for post in userPosts {
            ...
            if !post.postImageURL.hasPrefix("http") && !post.postImageURL.isEmpty {
                WLDStorageWorker.shared.deleteFile(name: post.postImageURL)
            }
            
            // Delete video file if it exists
            if let videoName = post.videoName, !videoName.isEmpty {
                WLDStorageWorker.shared.deleteFile(name: videoName)
            }
        }
