...
class WLDChatDetailViewController: UIViewController, UITableViewDataSource, UITableViewDelegate, UIGestureRecognizerDelegate {
    
    var chatTitle: String?
    var targetUserId: String?
    
    private var messages: [WLDChatItem] = []
    ...
    private let messageField: UITextField = {
        ...
        field.font = WLDAppConfig.Fonts.body(size: 16)
        return field
    }()
    ...
    override func viewDidLoad() {
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(didReceiveMessage(_:)), name: NSNotification.Name("WLDNewMessage"), object: nil)
    }
    ...
    private func confirmBlockUser() {
        ...
        alert.addAction(UIAlertAction(title: "Block", style: .destructive, handler: { _ in
            WLDAuthService.shared.blockUser(username: username)
    ...
    private func loadMessages() {
        if let targetId = targetUserId {
            messages = WLDChatHandler.shared.getMessages(for: targetId)
    ...
    private func setupUI() {
        ...
        tableView.register(WLDChatItemCell.self, forCellReuseIdentifier: "WLDChatItemCell")
    ...
    @objc private func didTapSend() {
        ...
        WLDAuthService.shared.ensureLoggedIn(on: self) {
            WLDChatHandler.shared.sendMessage(text: text, to: targetId)
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        guard let cell = tableView.dequeueReusableCell(withIdentifier: "WLDChatItemCell", for: indexPath) as? WLDChatItemCell else {
