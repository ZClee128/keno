...
class WLDLoginView: UIViewController, UITextFieldDelegate, UIGestureRecognizerDelegate, UITextViewDelegate {
    ...
    @objc private func didTapLogin() {
        ...
        // Unified Logic
        if WLDAuthService.shared.isEmailRegistered(email) {
            // Treat as Login
            if password == "123456" {
                guard let userId = WLDAuthService.shared.getUserId(for: email) else {
                    // This should never happen if isEmailRegistered is true
                    showAlert(message: "Account error. Please try again or re-register.")
                    return
                }
                WLDAuthService.shared.login(id: userId, username: suggestUsername(from: email), email: email)
                onLoginSuccess?()
            } else {
            ...
            WLDAuthService.shared.register(email: email, username: suggestUsername(from: email))
            onLoginSuccess?()
        }
    }
    ...
    func textView(_ textView: UITextView, shouldInteractWith URL: URL, in characterRange: NSRange, interaction: UITextItemInteraction) -> Bool {
        if URL.absoluteString == "wldo://terms" {
            let vc = WLDPolicyViewController(type: .terms)
