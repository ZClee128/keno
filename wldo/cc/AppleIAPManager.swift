...
class AppleIAPManager: NSObject {
    ...
    private override init() {
        ...
        // 监听应用将要销毁
        NotificationCenter.default.addObserver(self, selector: #selector(ox_5PoDx2m5),
                                               name: UIApplication.willTerminateNotification,
                                               object: nil)
    }

    // MARK: - NotificationCenter
    @objc func ox_5PoDx2m5() {
...
extension AppleIAPManager {
    ...
    ///   - productId: 产品Id
    ///   - block: 回调
    fileprivate func ox_bqYUveGz(productId: String, source: Int, handle: @escaping (String?, Bool) -> Void) {
        ...
        AppRequestTool.ox_O77YNHAa(model: reqModel) { succeed, result, errorModel in
    ...
    ///   - transaction: 交易信息
    ///   - params: 接口参数
    fileprivate func ox_rujX3TI9(_ transactionId: String, params: [String: String]) {
        let reqModel = AppRequestModel.init()
        ...
        AppRequestTool.ox_O77YNHAa(model: reqModel) { succeed, result, errorModel in
            guard succeed == true || errorModel?.errorCode == 405 else { // 验证接口失败，重试接口
                DispatchQueue.main.asyncAfter(deadline: DispatchTime.now() + 2) {
                    self.ox_b4ITsPD7(transactionId, .Pay)
                }
                return
            }
...
extension AppleIAPManager {
    ...
    ///   - productId: 产品Id
    ///   - block: 回调
    fileprivate func ox_hFRan57H(productId: String, source: Int, handle: @escaping (String?, Bool) -> Void) {
        let reqModel = AppRequestModel.init()
    ...
    ///   - transaction: 交易信息
    ///   - params: 接口参数
    fileprivate func ox_TYSVS737(_ transactionId: String, params: [String: String]) {
        let reqModel = AppRequestModel.init()
        ...
        AppRequestTool.ox_O77YNHAa(model: reqModel) { succeed, result, errorModel in
            guard succeed == true || errorModel?.errorCode == 405 else { // 验证接口失败，重试接口
                DispatchQueue.main.asyncAfter(deadline: DispatchTime.now() + 3) {
                    self.ox_b4ITsPD7(transactionId, .Subscribe)
                }
                return
            }
...
extension AppleIAPManager {
    /// 初始化数据
    private func ox_SReTA0ic() {
        self.payCacheList = ox_vGZBR5QH(payType: .Pay)
        self.subscribeCacheList = ox_vGZBR5QH(payType: .Subscribe)
    ...
    /// - Returns: 缓存列表
    private func ox_vGZBR5QH(payType: ApplePayType) -> [[String: String]] {
        var list: [[String: String]]?
        var diskPath = ""
        if payType == .Pay {
            diskPath = ox_nb43J7Rl()
        } else {
            diskPath = ox_uDU0OR1S()
        }
    ...
    /// 获取【购买】缓存路径【和uid关联】
    /// - Returns: 缓存路径
    private func ox_nb43J7Rl() -> String {
    ...
    /// 获取【订阅】缓存路径【和uid关联】
    /// - Returns: 缓存路径
    private func ox_uDU0OR1S() -> String {
        let documentDirectoryPath = NSSearchPathForDirectoriesInDomains(.documentDirectory, .userDomainMask, true).first ?? ""
        let appDirectoryPath = (documentDirectoryPath as NSString).appendingPathComponent("App")
    ...
    ///   - payType: 支付类型
    /// - Returns: 收据数据
    fileprivate func ox_xXQGNzvS(_ transactionId: String, _ payType: ApplePayType) -> String? {
        // 有未完成的订单，先取缓存
...
extension AppleIAPManager {
    ...
    func ox_5iq4EkG1() {
        ...
        for dict in self.payCacheList {
            ox_2ERxe0Lv(dict["transactionId"], .Pay)
        }
        
        // 【订阅】失败重试
        for dict in self.subscribeCacheList {
            ox_2ERxe0Lv(dict["transactionId"], .Subscribe)
        }
    }
    ...
    ///   - transactionId: Id
    ///   - payType: 支付类型
    private func ox_2ERxe0Lv(_ transactionId: String?, _ payType: ApplePayType) {
        guard let transactionId = transactionId else { return }
        // 初始化每个交易请求次数
        reqRetryCountDict[transactionId] = 0
        // 3. 服务端校验流程
        ox_b4ITsPD7(transactionId, payType)
    }
}
...
extension AppleIAPManager {
    ...
    func ox_ovkvFovu(productId: String, payType: ApplePayType, source: Int = 0, handle: @escaping IAPcompletionHandle) {
        ox_SReTA0ic()
        ...
        switch(payType) {
        case .Pay:
            ox_bqYUveGz(productId: productId, source: source) { [weak self] orderId, succeed in
                ...
                self.ox_ZNwKhaje(productId)
            }
        
        case .Subscribe:
            ox_hFRan57H(productId: productId, source: source) { [weak self] orderId, succeed in
                guard let self = self else { return }
                guard succeed == true && orderId != nil else { // 订单创建失败
    ...
    fileprivate func ox_ZNwKhaje(_ productId: String) {
...
extension AppleIAPManager: SKPaymentTransactionObserver {
        ...
        for transaction in transactions {
            ...
            case .purchased:   // 交易成功
                ...
                if transaction.original != nil && createOrderId == nil { // 启动自动续订时，不需要调用服务端验证接口
                    ...
                    // 3. 服务端校验流程
                    ox_b4ITsPD7(transaction.transactionIdentifier!, self.currentPayType)
                }
                // 移除苹果支付系统缓存
    ...
    ///   - payType: 支付类型
    fileprivate func ox_b4ITsPD7(_ transactionId: String, _ payType: ApplePayType) {
        guard let receiptStr = ox_xXQGNzvS(transactionId, payType) else {
        ...
        case .Pay:
            ...
            ox_rujX3TI9(transactionId, params: paramsArr.first!)
            
        case .Subscribe:
            let paramsArr = self.subscribeCacheList.filter({$0["transactionId"] == transactionId})
            guard paramsArr.count > 0 else { return }
            ox_TYSVS737(transactionId, params: paramsArr.first!)
        }
    }
}
