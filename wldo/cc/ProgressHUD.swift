    ...
    class func show(superView: UIView?) {
        if superView != nil {
            ...
            DispatchQueue.main.async {
                ...
                ProgressHUD.shared.activityIndicator.center = ProgressHUD.shared.center
                AppConfig.ox_uZ8RPXEH().addSubview(ProgressHUD.shared)
            }
        }
        ProgressHUD.shared.ox_Jir86xXM()
    }
    class func dismiss() {
        ProgressHUD.shared.ox_Ww21HE3h()
    }
    
    private func ox_Jir86xXM() {
        DispatchQueue.main.async {
            self.backgroundColor = UIColor(white: 0, alpha: 0)
...
extension ProgressHUD {
    class func ox_ngJ12YuQ(_ str: String?) {
        ox_ngJ12YuQ(str, showTime: 1)
    }
    class func ox_ngJ12YuQ(_ str: String?, showTime: CGFloat) {
