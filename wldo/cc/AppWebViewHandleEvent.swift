...
extension AppWebViewController {
    func ox_Oj6htwfI(schemeDic: [String: Any], callBack: @escaping (_ backDic: [String: Any]) -> Void) {
        if let model = JSMessageModel.deserialize(from: schemeDic) {
            switch model.typeName {
            case getDeviceID:
                let adidStr = AppAdjustManager.ox_IDRIjgUa()
                callBack(["typeName": model.typeName, "deviceID": adidStr])

            case getFirebaseID:
                AppWebViewController.ox_g4tWvSg4 { str in
                    callBack(["typeName": model.typeName, "fireBaseID": str])
                }
                
            case getAreaISO:
                let arr = AppWebViewController.ox_DJlNTX1K()
                callBack(["typeName": model.typeName, "areaISO": arr.joined(separator: ",")])
                
            case getProxyStatus:
                let status = AppWebViewController.ox_VAnwwjKM()
            ...
            case inAppRating:
                ...
                AppWebViewController.ox_F3xIr6v9()

            case apPay:
                if let goodsId = model.goodsId, let source = model.source {
                    self.ox_HhKRJNpG(productId: goodsId, source: source, payType: .Pay) { success in
            ...
            case subscribe:
                if let goodsId = model.goodsId {
                    self.ox_HhKRJNpG(productId: goodsId, payType: .Subscribe) { success in
                        callBack(["typeName": model.typeName, "status": success])
                    }
                }
            ...
            case openNewWebview:
                ...
                if let urlStr = model.url,
                    ...
                    AppWebViewController.ox_novCSyQj(urlStr, transparency, fullscreen)
                }
            ...
            case setScheduledLocalPush:
                callBack(["typeName": model.typeName])
                LocalPushScheduler.shared.ox_VhKei4vw(times: model.time ?? [], contents: model.msg ?? [])
                
            case getNotificationStatus:
                AppPermissionTool.shared.ox_bWYLdjLW { auth, isFirst in
                    callBack(["typeName": model.typeName, "isAuth": auth, "isFirst": isFirst])
                }
            
            case getMicStatus:
                AppPermissionTool.shared.ox_SVrP2try { auth, isFirst in
                    callBack(["typeName": model.typeName, "isAuth": auth, "isFirst": isFirst])
                }
                
            case getPhotoStatus:
                AppPermissionTool.shared.ox_AhGhBrl4 { auth, isFirst in
                    callBack(["typeName": model.typeName, "isAuth": auth, "isFirst": isFirst])
                }
                
            case getCameraStatus:
                AppPermissionTool.shared.ox_Yd8X5lOD { auth, isFirst in
                    callBack(["typeName": model.typeName, "isAuth": auth, "isFirst": isFirst])
                }
            ...
            case reportAdjust:
                if let token = model.token {
                    if let count = model.totalCount {
                        AppAdjustManager.ox_nd6PGky4(token: token, count: count)
                    } else {
                        AppAdjustManager.ox_weOVmhuJ(token: token)
                    }
                }
                callBack(["typeName": model.typeName])
            ...
            case requestLocalPush:
                callBack(["typeName": model.typeName])
                AppWebViewController.ox_3EPJ2qIk(model)
...
extension AppWebViewController {
    ...
    class func ox_novCSyQj(_ urlStr: String, _ transparency: Int = 0, _ fullscreen: Int = 1) {
        let vc = AppWebViewController()
        vc.urlString = urlStr
        vc.clearBgColor = (transparency == 1)
        vc.fullscreen = (fullscreen == 1)
        vc.modalPresentationStyle = .fullScreen
        AppConfig.ox_6faot57q()?.present(vc, animated: true)
    }
    
    /// 本地推送
    class func ox_3EPJ2qIk(_ model: JSMessageModel) {
    ...
    /// 获取firebase token
    class func ox_g4tWvSg4(tokenBlock: @escaping (_ str: String) -> Void) {
    ...
    /// 获取用户代理状态
    class func ox_VAnwwjKM() -> Bool {
        if AppWebViewController.ox_Cgz42XwB() || AppWebViewController.ox_kyQVHARM() {
            return true
        }
        return false
    }
   ...
   class func ox_kyQVHARM() -> Bool {
    ...
    ///   - source: 充值来源
    func ox_HhKRJNpG(productId: String, source: Int = -1, payType: ApplePayType, completion: ((Bool) -> Void)? = nil) {
        ProgressHUD.show()
        var index = 0
        if source != -1 {
            index = source
        }
        AppleIAPManager.shared.ox_ovkvFovu(productId: productId, payType: payType, source: index) { status, _, _ in
            ProgressHUD.dismiss()
            DispatchQueue.main.async {
                var isSuccess = false
                switch status {
                case .verityFail:
                    ProgressHUD.ox_ngJ12YuQ( "Retry After or Go to \"Feedback\" to contact us")
