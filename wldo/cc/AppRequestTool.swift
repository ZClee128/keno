...
@objc class AppRequestTool: NSObject {
    ...
    ///   - model: 请求参数
    ///   - completion: 回调
    class func ox_O77YNHAa(model: AppRequestModel, completion: @escaping FinishBlock) {
        let serverUrl = self.ox_GJLKTD9H(model: model)
        let headers = self.ox_mrHtv9W5(model: model)
        AF.request(serverUrl, method: .post, parameters: model.params, headers: headers, requestModifier: { $0.timeoutInterval = 10.0 }).responseData { [self] responseData in
            ...
            case .success:
                ox_7MMBDBlf(model: model, response: responseData.response!, responseData: responseData.data!, completion: completion)
    ...
    class func ox_7MMBDBlf(model: AppRequestModel, response: HTTPURLResponse, responseData: Data, completion: @escaping FinishBlock) {
...
//                    NotificationCenter.default.post(name: DID_LOGIN_OUT_SUCCESS_NOTIFICATION, object: nil, userInfo: nil)
    ...
    class func ox_GJLKTD9H(model: AppRequestModel) -> String {
    ...
    /// - Returns: 请求头参数
    class func ox_mrHtv9W5(model: AppRequestModel) -> HTTPHeaders {
